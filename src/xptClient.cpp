...
xptClient_t* xptClient_connect(generalRequestTarget_t* target, uint32 payloadNum)
{
	...
	xptClient->payloadNum = max(1U, min(127U, payloadNum));
...
{
	uint32 usernameLength = min(127UL, strlen(xptClient->username));
	uint32 passwordLength = min(127UL, strlen(xptClient->password));
	// build the packet
